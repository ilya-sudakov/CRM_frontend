...
const SelectContacts = (props) => {
    ...
    const clickOverlay = (event) => {
        const overlay = document.getElementsByClassName("select-contacts__overlay")[0];
    ...
    const clickOnForm = (e) => {
        ...
        const form = document.getElementsByClassName("select-contacts__selected_form")[id];
        if (form.classList.contains("select-contacts__selected_form--hidden")) {
            e.target.type !== 'text' && !e.target.classList.contains("select-contacts__img") && form.classList.remove("select-contacts__selected_form--hidden");
        }
        else {
            e.target.type !== 'text' && !e.target.classList.contains("select-contacts__img") && form.classList.add("select-contacts__selected_form--hidden");
        }
    }
    ...
    return (
        <div className="select-contacts">
            <div className="select-contacts__overlay select-contacts__overlay--hidden" onClick={clickOverlay}></div>
            {!props.readOnly &&
            ...
            <div className="select-contacts__selected">
                {selected.map((item, index) => (
                    <div className="select-contacts__selected_item" >
                        <div className="select-contacts__selected_header" index={index} onClick={clickOnForm}>
                            <div className="select-contacts__selected_name">
                                <span>ФИО: </span> <span>{item.lastName + ' ' + item.name}</span>
                            </div>
                            <div className="select-contacts__selected_name">
                        ...
                        <div className="select-contacts__selected_form select-contacts__selected_form--hidden" >
                            ...
                            <div className="select-contacts__item">
                                <div className="select-contacts__input_name">Номер телефона</div>
                                <div className="select-contacts__input_field">
                                    <input
                                        type="text"
                                        name="phoneNumber"
                        ...
                        {!props.readOnly && (index !== 0) &&
                            <img index={index} onClick={deleteContact} className="select-contacts__img" src={deleteSVG} />
                        }
                    </div>
                ))}
            </div>
